...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=('Generates a comma separated file containing information '
                     'about files and folders of given directory'))
    parser.add_argument('-d', '--directory', type=check_directory, required=True,
...
def append_record(data, file_path, files):
    for filename in files:
        ...
        if path.islink(file):
            ...
            filetype = 'd'
            mod_time = datetime.fromtimestamp(path.getmtime(file)).strftime('%Y-%m-%d-%H:%M')
        else:
            stat = os.stat(file)
...
def write_data(destination_dir, destination_file, data):
    ...
    with open(path.join(destination_dir, destination_file), 'w', encoding='utf8') as csv_file:
